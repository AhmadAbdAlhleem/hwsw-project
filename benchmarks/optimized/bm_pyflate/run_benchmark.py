...
# OPTIMIZATION 1: precomputed bit masks.
# The original recomputed (1 << n) - 1 inside every readbits()/snoopbits() call.
# Those two methods are the innermost primitives of the whole decoder, so this
# shift+subtract ran millions of times per benchmark iteration.
_MASK = tuple((1 << n) - 1 for n in range(257))


class BitfieldBase(object):
    # OPTIMIZATION 2: __slots__ removes the per-instance __dict__ and makes
    # every self.bits / self.bitfield access a slot load instead of a dict probe.
    __slots__ = ('f', 'bits', 'bitfield', 'count')
...
class Bitfield(BitfieldBase):
    ...
    def snoopbits(self, n=8):
        if n > self.bits:
            self.needbits(n)
        return self.bitfield & _MASK[n]

    def readbits(self, n=8):
        if n > self.bits:
            self.needbits(n)
        r = self.bitfield & _MASK[n]
        self.bits -= n
        self.bitfield >>= n
        return r
...
class RBitfield(BitfieldBase):
    ...
    def snoopbits(self, n=8):
        if n > self.bits:
            self.needbits(n)
        return (self.bitfield >> (self.bits - n)) & _MASK[n]

    def readbits(self, n=8):
        if n > self.bits:
            self.needbits(n)
        bits = self.bits - n
        r = (self.bitfield >> bits) & _MASK[n]
        self.bits = bits
        self.bitfield &= ~(_MASK[n] << bits)
        return r
...
class HuffmanLength(object):
    # OPTIMIZATION 3: there is one HuffmanLength per symbol per table (hundreds
    # of them, rebuilt for every compressed block), and the decoder touches
    # .bits / .reverse_symbol / .code on each one. __slots__ drops the
    # per-instance dict entirely.
...
class HuffmanTable(object):
    ...
    def populate_huffman_symbols(self):
        ...
        # OPTIMIZATION 4 (the big one): build O(1) decode tables.
        #
        # The original find_next_symbol() linearly scanned self.table for EVERY
        # symbol decoded, comparing x.reverse_symbol against freshly snooped
        # bits. self.table holds one entry per symbol -- up to 288 for a DEFLATE
        # literal/length table -- so decoding one symbol cost O(table size) with
        # three attribute loads per entry.
        #
        # self.table is sorted by (bits, code), i.e. by code length ascending,
        # and within one code length every symbol is distinct. So the scan is
        # exactly equivalent to: for each code length in ascending order, look
        # up the snooped bits. We precompute one dict per code length, turning
        # the inner loop into a single hash lookup and reducing the probe count
        # from O(number of symbols) to O(number of distinct code lengths) (<= 15
        # for DEFLATE, <= 20 for bzip2).
        rev = {}
        fwd = {}
        for x in self.table:
            b = x.bits
            if b not in rev:
                rev[b] = {}
                fwd[b] = {}
            rev[b][x.reverse_symbol] = x.code
            fwd[b][x.symbol] = x.code
        self._rev_by_len = rev
        self._fwd_by_len = fwd
        self._code_lengths = sorted(rev)
    ...
    def find_next_symbol(self, field, reversed=True):
        # Table-driven decode -- see populate_huffman_symbols() for why this is
        # equivalent to the original linear scan.
        by_len = self._rev_by_len if reversed else self._fwd_by_len
        snoopbits = field.snoopbits
        for bits in self._code_lengths:
